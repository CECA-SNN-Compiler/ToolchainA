...
class TestFull(nn.Module):
    def __init__(self,channel,dataset="CIFAR10",scale=1):
        super().__init__()
        if dataset == "CIFAR10":
            nunits_input = 3
            outputs = 10
            nuintis_fc = int(channel * scale) * 4*4
        else:
            raise NotImplementedError

        self.conv1 = SpikeConv2d(nunits_input, int(channel * scale), 5,bias=False)
        self.relu1= SpikeReLU()
        self.conv2 = SpikeConv2d(int(channel * scale), int(channel * scale), 3,bias=False)
        self.relu2 = SpikeReLU()
        self.conv3 = SpikeConv2d(int(channel * scale), int(channel * scale), 3,bias=False)
